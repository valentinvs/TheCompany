    ...
    internal sealed class Configuration : DbMigrationsConfiguration<TheCompanyDbContext>
    {
        ...
        protected override void Seed(TheCompanyDbContext context)
        {
            try
            {
                ...
                var rootMenuFood = CreateMenu(theCompanyDb, "Food", "Ястия", null);

                var menuStarters = CreateMenu(theCompanyDb, "Starters", "Предястия", rootMenuFood.Id);
                ...
                var menuSalads = CreateMenu(theCompanyDb, "Salads", "Салати", rootMenuFood.Id);
                ...
                var menuSnacks = CreateMenu(theCompanyDb, "Snacks", "Плата", rootMenuFood.Id);
        ...
        private static Menu CreateMenu(TheCompanyDbContext dbContext, string titleEN, string titleBG, int? parentMenuId, string picturePath = null, string thumbnailPath = null)
        {
            ...
            {
                return dbContext.Menus.FirstOrDefault(m => m.TitleEN == menu.TitleEN && m.Deleted == false);
            }
        }
        ...
        private static void CreateContactInfo(TheCompanyDbContext theCompanyDb, string titleEN, string titleBG, string descriptionEN, string descriptionBG)
        {
            ...
            contactInfo.ContactLabelEN = titleEN;
            contactInfo.ContactLabelBG = titleBG;
            contactInfo.ContactDataEN = descriptionEN;
            contactInfo.ContactDataBG = descriptionBG;

            if (!theCompanyDb.ContactInfos.Any(c => c.ContactLabelEN == contactInfo.ContactLabelEN))
            {
